...
description: Quick-reference sheet of verification methodologies for AI-generated code and agentic systems (type checking, static analysis, formal verification, evals, sandboxing, red-teaming, and more), plus the Trust Spec T/A/I/D/U classification framework. Use this skill whenever deciding how to verify a piece of code or an agent's output, designing a quality gate, classifying a requirement's trust level, comparing two verification approaches, or writing/updating docs/verificators.md — even if the request doesn't name a specific method.
---

# Verification Methodologies — Reference Sheet

A quick-reference list of verification methodologies for AI-generated code and agentic systems, each with a plain definition and a link to an explanation of the methodology itself (not a product page).

This is general vocabulary, not a StoryMaker-specific prescription — it doesn't say which methods StoryMaker actually uses. For that, and for the Spanish-language elaboration with StoryMaker's own reasoning about how the framework applies, see `docs/verificators.md`, which this sheet is the source reference for.
...
| Property-based testing | Specifying a general property, then generating many inputs to search for a violation | [QuickCheck: A Lightweight Tool for Random Testing of Haskell Programs — Claessen & Hughes, 2000](https://dl.acm.org/doi/10.1145/351240.351266) |
...
Note: Property-based testing and evals don't have a single neutral "founding" reference the way formal verification does. The links above point to the paper that introduced or formalized the methodology in each case (QuickCheck for the former, HELM for the latter), not the only possible choice.
