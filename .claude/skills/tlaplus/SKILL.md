...
`.claude/skills/tlaplus/scripts/tlc.sh`, no separate install step needed.
...
1. Write `Spec.tla` (the spec/module) and `Spec.cfg` (which invariants to
   ...
   it never finishes).
2. Run: `bash .claude/skills/tlaplus/scripts/tlc.sh Spec.tla Spec.cfg`
...
6. **For every confirmed bug, pin it with a test that fails before the fix
   ...
   the draft and wait for the human's OK.
