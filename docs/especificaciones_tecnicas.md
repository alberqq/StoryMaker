...
| 2.10 | 1.9 | `cerrar_unidad` deja constancia del cierre en `unidades.jsonl`, que hasta ahora solo se escribia al abrir y dejaba toda unidad en `en_curso` para siempre. Y el tiempo de las paradas del Autor se contabiliza aparte del tiempo de trabajo en la observabilidad: el presupuesto ya no lo contaba, pero los informes si |
| 2.11 | 1.9 | Cada despacho de subagente se emite como span de tipo `agent`, hijo del tramo que lo lanzo. Antes la traza eran tres cajas y el gasto solo se atribuia al modelo: un `tramo_novela` de seis euros no decia si se fueron en redactar, en refinar o en un capitulo devuelto cuatro veces |
| 2.12 | 1.9 | La traza se identifica por el **Proyecto** y no por el proceso: una novela relanzada tres veces era tres trazas distintas con el mismo nombre y sin forma de distinguirlas. Ahora es una sola traza con una raiz por tirada, fechada, y los identificadores de span llevan el proceso dentro para que dos tiradas no se pisen |
| 2.13 | 1.9 | El consumo se atribuye a quien lo gasta y no al modelo que lo sirve, leyendo `parent_tool_use_id` del flujo; se acumula segun llega, de modo que una etapa muerta conserva sus tokens; se escribe en `tmp/consumo/` antes que en Langfuse; y los spans que no se pueden exportar se apartan en `tmp/trazas_sin_enviar/` en lugar de perderse en silencio |
| **2.14** | **1.9** | **Vigente.** Las sesiones que lanza la interfaz declaran `STORYMAKER_PROYECTO`. Sin ella, y con mas de un Proyecto en disco, los cinco hooks que dependen del Proyecto resolvian `None` y permitian sin comprobar: el segundo mecanismo de ADR-02 se apagaba solo en cuanto habia una segunda novela. Se descubrio porque las unidades de trabajo no se cerraban nunca |
