...
| Lean sobre la escaleta | Una escaleta que pone a un personaje en dos lugares el mismo día deja un aviso en el gate de Plotting, sin haber redactado nada, con Lean o, sin `lake`, con la evaluación en Python |
| Rehacer la Trama | Tras «rehacer» en el gate de Plotting el arquitecto vuelve a planificar con el comentario, los avisos y la trama anterior, y la trama se sustituye sin duplicarse |
...
| U-13 | **Fiabilidad del verificador de respaldo.** Es un modelo decidiendo si un fragmento sostiene un enunciado —y, en el veredicto parcial, qué es el dato central y qué un añadido—, y puede equivocarse en ambas direcciones | Montar un juego de pares etiquetados a mano para un validador que no bloquea nada cuesta más de lo que vale. Su clase de confianza es **I**: quien decide es el Autor en el gate | El veredicto no gobierna ninguna arista del grafo; solo limita la firmeza del hecho y colorea el informe de G4. El Autor tiene el enunciado, la cita y la URL delante | Si el informe resultara tan ruidoso que el Autor dejara de mirarlo |
