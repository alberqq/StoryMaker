...
- `guardrail_prohibidas`: un caso por nivel (`global`, `novela`, `destinatario`), más variantes con acento, plural y mayúsculas.
...
| `anclas_de_procedencia` | Las anclas `spec:` y `arq:` de los docstrings contra los documentos, y §3 y §4 de la spec contra los símbolos que los citan | T | No |
...
| U-17 | **La API de lectura no tiene autenticación.** Cualquiera que alcance el puerto puede leer una novela y enviar una petición de cambio | Montar usuarios y sesiones cuesta más que el riesgo que cubre en un sistema que corre en local y no se expone a internet | El webhook de Telegram —el único endpoint que reanuda una ejecución— sí comprueba su `secret_token`, de modo que lo que queda abierto es leer y pedir un cambio, y una petición de cambio no toca nada hasta que el Autor la aprueba en el gate de Regeneration | Si el backend se desplegara fuera de la máquina del Autor |
...
| **G3 · Capítulo** | Tras cada `WriteChapter`, en ejecución, en dos pasadas | **Pasada determinista:** los validadores programáticos de §11a que actúan sobre el capítulo. **Pasada del extractor**, solo si la anterior queda limpia: `cobertura_capitulo` y los cuatro invariantes de Lean sobre la cronología acumulada, más `ejecucion_escaleta` y `arco_ejecutado` en calidad de aviso | Filas en `incidencia` y `score`, *scores* en la traza | Lo bloqueante vuelve al editor; agotados los dos reintentos, `Fail`. Los avisos no detienen el capítulo: viajan al encargo del siguiente y al informe de G4 |
