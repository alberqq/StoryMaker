...
- **Un elemento obligatorio sin anclar se ancla solo** a la escena que más se le parece por embeddings, y el aviso dice a cuál. Así llega al paquete del escritor con su escena, que es donde el escritor mira lo que tiene que usar; `cobertura_capitulo` y `cobertura_personalizacion` siguen detrás para comprobar que se escribió.
...
**En el gate de Plotting, sin `lake`, los mismos invariantes se evalúan en Python** sobre los mismos datos que el generador vuelca, y también si Lean falla por avería. Ahí es un aviso y no una puerta, así que no hace falta la garantía de Lean para que sirva; en la pasada del extractor y en la publicación, donde sí bloquea, sigue siendo Lean o nada. Quien no tiene fecha de nacimiento no nace el día de la época: se le da un nacimiento tan temprano que ninguna escena cae antes, porque «no se sabe» no puede convertirse en «nació en 1800» en una novela del siglo XVI.
