...
| **H5** | Verificación de extremo a extremo, trazabilidad en G1 y entrega | G1, G2, G5 |
...
| **IMP-04** | **Steiger** como script que corre en la etapa de frontend de G1 e **informa y no bloquea** | `frontend/package.json::scripts.estructura` | La etapa enseña la salida de `npx steiger src` y **no falla aunque haya hallazgos** | §16.1, §16.3, verif. §3.2 | G1 |
...
| # | Entregable | Ficheros y símbolos | Criterio de hecho | Arq. | Gate |
|---|---|---|---|---|---|
| **IMP-28** | Pruebas de componente e integración con **MSW** sobre los contratos de `shared/api`, incluidos los siete casos de error de §9 de la spec | `frontend/tests/**` | Los siete casos de error tienen prueba, y la suite corre en la misma etapa de G1 que Steiger | verif. §3.5 | G1 |
...
| **IMP-53** | **La maqueta de libro** de la ruta de impresión: portada a página completa, nota del autor e índice en su página, capítulos en página nueva con encabezado y capitular, texto justificado, y en pantalla el documento como páginas | `app/styles/print.css`, `pages/print/ui/Documento.tsx`, `pages/print/ui/Portada.tsx`, `pages/print/ui/Indice.tsx` | Las pruebas de impresión de H4 siguen en verde; el PDF de una novela real tiene portada, índice y un capítulo por página nueva | §16.1 | G5 |
| **IMP-54** | **Elegir el candidato de Regeneración**: los candidatos como radios con su alcance, la casilla «Valor nuevo» precargada con el valor actual del elegido, y aprobar con `<objeto>:<fila_id> <campo>=<valor>`; rehacer sigue con el comentario libre | `pages/gate/ui/Regeneracion.tsx`, `pages/gate/ui/Decision.tsx`, `pages/gate/ui/Gate.tsx`, `pages/gate/model/eleccion.ts`, `frontend/tests/pantallas/gate.test.tsx` | Elegir el segundo candidato y cambiar su valor envía exactamente su `objeto:fila_id campo=valor` | §4 F6 | G1 |
| **IMP-55** | **Las publicadas, en un listado bajo el tablero**: `COLUMNAS` pierde `publicadas`, `enListado` decide qué novela sale del tablero, y `Publicadas` pinta la tabla con su enlace de lectura y, en Regeneración, al gate | `pages/library/model/columnas.ts::enListado`, `pages/library/ui/Publicadas.tsx`, `pages/library/ui/Tablero.tsx`, `pages/library/ui/tablero.css`, `frontend/tests/pantallas/taller.test.tsx` | Una novela publicada no es tarjeta de ninguna columna y sale como fila del listado; el tablero tiene cinco columnas | §16.5 | G1 |
| **IMP-56** | **Ajustes de lectura y sin «Imprimir»**: `usePreferenciasDeLectura` en `shared/lib`, el único módulo que toca `localStorage`, con el tamaño y la fuente; `AjustesDeLectura`, la pestaña «Aa», en `shared/ui`, montada por `Pagina`, que aplica las preferencias como variables CSS del `.libro`; fuera los enlaces a `rutaImprimir` del índice de lectura y de la salida de la Publicación; `sin-copia-local` exime ese módulo y comprueba que solo guarda la clave de preferencias | `shared/lib/preferencias.ts`, `shared/ui/AjustesDeLectura.tsx`, `shared/ui/Pagina.tsx`, `app/styles/libro.css`, `pages/reading/ui/Indice.tsx`, `pages/phase/ui/SalidaPublicacion.tsx`, `frontend/tests/estructura/sin-copia-local.test.ts`, `frontend/tests/pantallas/lectura.test.tsx` | Cambiar tamaño y fuente cambia el texto en el acto y sobrevive a volver a montar la pantalla; ninguna pantalla enlaza a `/imprimir` | §16.3 | G1 |
| **IMP-57** | **Temas e interlineado**: `preferencias.ts` pasa a ser un almacén con `useSyncExternalStore` —una sola clave, la leen a la vez la barra y el libro— que guarda también interlineado, negrita, `temaLibro` y `temaApp`, y pone `data-tema` en `<html>` resolviendo «sistema» con `matchMedia`; `tokens.css` define el juego claro y el oscuro para `:root[data-tema]` y para `[data-tema-libro]`, y conserva la media query para antes de que cargue el script; `SelectorDeTema` en la barra; `Pagina` pone `data-tema-libro` cuando el lector lo fija | `shared/lib/preferencias.ts`, `shared/ui/AjustesDeLectura.tsx`, `shared/ui/SelectorDeTema.tsx`, `shared/ui/Marco.tsx`, `shared/ui/Pagina.tsx`, `app/styles/tokens.css`, `app/styles/libro.css`, `frontend/tests/pantallas/lectura.test.tsx` | Con la aplicación en oscuro y el libro en claro, `<html>` lleva `data-tema="oscuro"` y el libro `data-tema-libro="claro"`; el interlineado y la negrita cambian el texto en el acto | §16.3 | G1 |
...
| Fecha | Cambio | Motivo |
|---|---|---|
| 2026-09-24 | **IMP-04** deja de nombrar `.github/workflows/ci.yml` y pasa a ser una etapa de G1 lanzada a mano; IMP-28, IMP-33 y H5 dicen «en G1» en lugar de «en CI» | Decisión del Autor: se retiran los workflows de GitHub Actions y G1 y G2 pasan a lanzarse a mano (arq. §17, verif. §4.7) |
| 2026-09-24 | Entra **IMP-57**: temas de la aplicación y del libro, interlineado y negrita | Baja de §4.6 y §8 de la spec |
| 2026-09-24 | Entra **IMP-56**: ajustes de lectura y el lector sin «Imprimir»; IMP-32 exime el módulo de preferencias | Baja de §1 y §4.6 de la spec |
| 2026-09-24 | Entra **IMP-55**: las publicadas en un listado bajo el tablero; IMP-39 pasa a cinco columnas | Baja de §4.1 de la spec |
| 2026-09-24 | Entra **IMP-54**: el candidato de Regeneración elegido en el gate | Baja de §4.4 de la spec |
| 2026-09-24 | Entra **IMP-53**: la maqueta de libro de la impresión | Baja de §7 de la spec |
