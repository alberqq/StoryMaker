...
La lista de validadores está en §7.2, y esa tabla, la de §11a de la arquitectura y el registro se comparan por pares en G1.
...
**El navegador ve la versión candidata.** `render_visual` abre en Chromium el HTML de lectura armado con la lista exacta de `capitulo_version` que la versión va a tener —`construir_lectura_candidata`—, el mismo con el que se imprime el PDF de respaldo. Comprueba que portada, índice y ficha de personajes se pintan y pulsa cada enlace del índice; lo que no se ve cita su pieza o su capítulo. Sin navegador, queda la comprobación estructural y un aviso. La interfaz de React no se abre aquí porque la versión aún no existe y la API no puede servirla; se inspecciona aparte sobre versiones publicadas (arq. §11a, U-21).

**El render se comprueba antes de publicar.** La cronología completa y `render_visual` corren **antes de escribir ninguna fila de la versión**, las dos siempre y las dos con su *score*. Si alguna trae bloqueantes no hay nada que deshacer: el rechazo queda como incidencia que cita capítulos y `publication.publish` devuelve la novela al gate de Writing por la arista `PublishVersion → AwaitApproval4`, con el tope del juez, o a `Fail` si se agotó. G5 no admite excepción: no hay camino que escriba la versión sin las dos en verde. El detalle está en la [spec de validación](../validacion/spec.md).

**Sin gate humano**, porque el manuscrito ya se aprobó al cerrar Writing y lo que queda es automático.

**El juez recibe la rúbrica** —las ocho preguntas de `rubrica.yaml`, el mismo fichero de la revisión humana— antepuesta a la novela. **Antes de puntuar enumera las contradicciones**: lo que un capítulo afirma y otro desmiente, y lo que un personaje sabe o cuenta antes de que ocurra en la fecha narrativa. `SalidaJuez.contradicciones` las recoge, y la nota de continuidad **se topa en Python** a `10 − 2·n`, con mínimo 1; el juez las cuenta, pero no decide cuánto pesan. Las contradicciones se guardan en el detalle del *score*. **El PDF** se imprime **desde la ruta de impresión del frontend** (arq. §16.1), junto al fichero de la novela como `<novela>.v<n>.pdf`, y si falla queda como aviso: la versión ya está publicada y validada. Se imprime **cuando la invocación que publicó ya ha confirmado**, al salir del grafo y antes de avisar de que ha terminado, porque dentro del paso de `publish` la versión todavía no es visible para otra conexión. No hace falta un servidor levantado: Playwright abre `/novelas/<n>/v/<k>/imprimir` y el propio proceso le responde, con los estáticos de `frontend_dist` y la API con la aplicación de FastAPI en memoria. Se imprime toda versión que aún no tenga su PDF, de modo que una impresión fallida se repite en la siguiente invocación que termine. Sin `dist/` se cae al HTML mínimo de `render.py`, con un aviso.

**Errores.** Lean o `render_visual` fallan → **la versión no se publica, sin anulación posible**, y la novela vuelve al gate de Writing con los capítulos citados; agotados los rechazos, `Fail`. Umbral del juez no superado → vuelta al gate de Writing **con gates**; **en modo batch se registra la nota y se publica**, porque no hay nadie que decida qué rehacer.
...
**Notificación tras la interfaz `Notifier`, y solo para avisar**, con Telegram como única implementación hoy. El aviso de un gate lleva el título —la fase con el nombre de la interfaz, «Trama: espera tu decisión»—, **un resumen en cifras** de lo que hay que revisar y termina con **«Decide en el PC»**, en texto plano, **sin comandos y sin botones**. El resumen es una línea de recuentos por fase; en el gate de Intake, las preguntas del entrevistador enteras o, sin ellas, los avisos del brief; en el de Plotting, los huecos encontrados e inventados y los avisos de la revisión agrupados por tipo, lo grave primero. **El informe entero no va al móvil**: sale por la salida del proceso, que es donde se decide, y en la pantalla del gate. Si Telegram rechaza el envío o no hay red, se avisa en la salida del proceso y el gate bloquea igual. WhatsApp exige Meta Business, número verificado y plantillas aprobadas, y queda como adaptador futuro.
...
| `nombres_exactos` | Homenajeado y personajes escritos exactamente como en el canon; la inicial no cuenta como otra grafía en las dos direcciones: mayúscula a principio de frase, y minúscula en la primera palabra de un nombre de varias | Post `WriteChapter` | A | G3 | Sí |
...
| `arco_anclado` | Todo personaje presente en **≥3 escenas** de `plan_escena_personaje` tiene fila en `canon_arco`; si su arco es positivo o negativo, **≥2 hitos** anclados a escenas de capítulos que no retroceden: dos hitos pueden compartir capítulo. **El arco plano cuenta**: declarar que un personaje no se transforma es la decisión que el validador reclama. El homenajeado es la única excepción — no puede tener arco plano y su último hito cae en el tercio final | Gate de Plotting | A | G4 | Sí |
...
| `cobertura_personalizacion` | Cada elemento obligatorio aparece en ≥1 capítulo | Gate de Writing | A | G4 | Sí |
| `render_visual` | Portada, índice y ficha de personajes se pintan en Chromium, y cada enlace del índice lleva a su capítulo con texto | Dentro de `PublishVersion`, sobre la versión candidata, **antes de escribirla** | T | G5 | Sí |
...
| REQ-BE-46 | El registro, la tabla de §11a de la arquitectura y la de §7.2 de este documento se comparan **por pares** en G1 | §3.6 | P-132 |
...
| REQ-BE-88 | `render_visual` juzga en Chromium la **versión candidata**, armada con los `capitulo_version` que la versión va a tener, antes de escribirla | §4.5 | P-92 |
...
| Fecha | Cambio | Motivo |
|---|---|---|
| 2026-09-25 | §4.5: el navegador abre **la lectura candidata** y no la ruta de impresión interceptada; cronología y render corren **antes de escribir la versión**, y el rechazo vuelve al gate de Writing. El juez puntúa ocho criterios. §7.2: `render_visual` pasa a **T**, y `cobertura_personalizacion` a G4. REQ-BE-88 se reescribe con el mismo identificador | Propagación de arq. §9 y §11 y de la [spec de validación](../validacion/spec.md) |
| 2026-09-24 | §7.2: `nombres_exactos` admite la inicial en minúscula en un nombre de varias palabras. El bucle de Writing, la cronología que bloquea y las contradicciones del juez bajan a `specs/escritura/` | Se propagan §4 (Fase 4), §11a y §11c de la arquitectura |
| 2026-09-24 | §3.6, §7.1 y REQ-BE-46 dejan de decir «en CI» y dicen «en G1» | Decisión del Autor: se retiran los workflows de GitHub Actions y G1 y G2 pasan a lanzarse a mano (arq. §17, verif. §4.7) |
| 2026-09-24 | §4.7: el aviso de un gate resume en cifras y el informe entero no va al móvil; el título nombra la fase como la interfaz. §7.2: los hitos de `arco_anclado` van en capítulos que no retroceden | Se propagan §10, §11a y §19 de la arquitectura. El detalle de la Trama baja a `specs/trama-rehacible/` |
