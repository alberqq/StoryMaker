...
| **P-140** | **Avisos fuera de gate**, que salen siempre, también en batch: **parada** cuando la invocación termina en `Fail` —nodo o capítulo, motivo y los comandos `estado` y `continuar`—, **final** cuando llega a `Idle` —versión y coste— y **aparcamiento** cuando `aparcar` aparca un gate. Pasan por `avisar_sin_fallar`, así que un aviso perdido no cambia el resultado; parar en un gate no emite aviso de parada. `invocar` y `reanudar` aceptan un `notifier` inyectable | `gates/notifier.py::aviso_de_parada`, `gates/notifier.py::aviso_de_terminada`, `gates/notifier.py::aviso_de_aparcada`, `gates/notifier.py::avisar_sin_fallar`, `commons/graph/run.py::_avisar`, `gates/nodos.py::aparcar` | §10 | G1 |
