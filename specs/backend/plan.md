...
| **P-05** | Puerta G1, lanzada a mano en la máquina del Autor con los comandos de verif. §4.7, en tres bloques —estática, pruebas y seguridad—: `uv sync --frozen`, `ruff` con el conjunto `S` de bandit, `mypy --strict`, `pytest`, `gitleaks`, Semgrep y `pip-audit --skip-editable` sobre `uv.lock`. La configuración de `ruff` y de `mypy` vive en `pyproject.toml`, no en ficheros aparte | `backend/pyproject.toml` | §16.1, verif. §4.7, §6 | G1 |
...
| **P-129** | `inventario_del_plan`: parser de la columna «Ficheros y símbolos» de todo `specs/*/plan.md` **—filas `P-nn` y filas `IMP-nn`—** y cotejo contra el árbol en las dos direcciones —todo módulo de `backend/src/storymaker/**` salvo los `__init__.py`, y todo `.ts` o `.tsx` de `frontend/src/**`, tiene que estar declarado en algún ítem—, con la salida partida en **dos cubos**: «declarado y ausente», que se mira al cerrar hito, y «presente y no declarado», que se mira siempre | `tests/correspondencia/test_inventario.py` | §11e | G1 |
...
| **P-18** | Migraciones numeradas, versión de esquema en la novela y **negativa a abrir un esquema del futuro**; `mundo_hecho.sin_respaldo` entra como columna aditiva, al abrir y sin subir la versión (GI-18) | `commons/db/migraciones/`, `commons/db/apertura.py::migrar` | spec §2.3 | G1 |
...
| **P-31** | `schema_guard`: la salida se valida contra el esquema Pydantic del rol **antes de escribir en SQLite**, con reintento que inyecta el error de validación. Admite un **contexto de validación** —el dominio de identificadores del rol— que `invocar_rol` recibe y entrega a `model_validate` | `commons/agents/schema_guard.py`, `commons/agents/invocacion.py` | §11a | G3 |
...
| **P-150** | **Lo que ya ha pasado** en el bloque 3: tras el estado al cierre de N−1, los eventos narrativos de `cronologia_evento` de las versiones aprobadas de los capítulos 1 a N−2, recortados a 160 caracteres y del más reciente al más antiguo. Techos: continuidad 2.500, memoria 3.000 | `commons/context/bloques.py::continuidad`, `commons/db/repos/texto.py::eventos_anteriores`, `commons/context/paquete.py::TECHOS_DE_BLOQUE` | §6 | G1 |
| **P-151** | **Reglas de escritura y lo ya gastado** en el bloque 6: cuatro reglas fijas, y las palabras y expresiones más repetidas en los capítulos aprobados con su recuento, y la última frase de cada uno, calculadas en Python | `commons/context/bloques.py::reglas`, `commons/context/repeticion.py::mas_repetidas`, `commons/context/repeticion.py::ultima_frase` | §6 | G1 |
...
| **P-41** | Los cinco validadores de capítulo: `nombres_exactos` —que admite la mayúscula inicial de principio de frase—, `longitud_capitulo`, `guardrail_prohibidas` con derivadas por raíz y normalización de mayúsculas, acentos, plurales y variantes, `anacronismo_fechado` y `anclaje_valido` | `commons/validation/chapter_validator.py`, `commons/validation/policy_checker.py` | §11a, §15 | G3 |
...
| **P-63** | TLC sobre el modelo pequeño —5 capítulos, 2 reintentos— en G1, con los contraejemplos documentados junto al cambio que provocaron | `formal/tla/harness.cfg`, `docs/iteraciones.md` | §11d | G1 |
| **P-141** | **Una `fase_run` por fase**, abierta por un envoltorio que `nodos_resueltos` pone a los veinticuatro nodos con la fase de cada uno declarada en `FASE_DE_NODO`; gates y terminales heredan la fila abierta. Se abre fila nueva cuando la fase cambia o cuando la abierta ya no está `en_curso`, que es rehacer desde un gate. `invocar` cierra la fila abierta al salir —`esperando_gate`, `completada` o `fallida`— también tras reanudar. El estado lleva `corpus_run_id`, y el corpus se verifica, se completa y se sella por él. Detalle en [`specs/ejecucion-real/plan.md`](../ejecucion-real/plan.md) | `commons/graph/nodos.py::FASE_DE_NODO`, `commons/graph/contabilidad.py::contabilizado`, `commons/graph/contabilidad.py::corpus_de`, `commons/graph/construccion.py::nodos_resueltos`, `commons/graph/run.py::invocar`, `commons/db/repos/arnes.py::fase_run_abierta`, `commons/db/repos/arnes.py::cerrar_abierta` | §8 | G1 |
| **P-142** | **El consumo lo cuenta el transporte**: `TransporteContado` envuelve al de producción o al doble y acumula cada `pedir`, reintentos de esquema incluidos; el envoltorio de P-141 suma la diferencia de cada nodo al estado y a la fila abierta con `sumar_consumo` | `commons/agents/contador.py::TransporteContado`, `commons/db/repos/arnes.py::sumar_consumo`, `commons/graph/run.py::invocar` | §8 | G1 |
...
| # | Entregable | Ficheros y símbolos | Arq. | Gate |
|---|---|---|---|---|
| **P-69** | Nodo `investigation.research`: **una sesión**, las seis dimensiones del período, con **3 `WebSearch` y 3 `WebFetch`** impuestas por los hooks de P-29 y P-30, cuyo motivo de denegación pide entregar ya; veinte turnos, y una sesión sin respuesta válida deja un aviso y la fase sigue | `investigation/nodos.py::research`, `investigation/esquemas.py`, `commons/agents/hooks.py::CuotaDeHerramientas`, `commons/agents/techos.py` | §4, §19 | G1 |
...
| **P-72** | Nodo `investigation.verify`, que realiza el validador semántico `respaldo_fuente`: agente **sin herramientas y sin red**, por **lotes de veinte**, que escribe **solo** `respaldo` y, en el veredicto parcial, `sin_respaldo`, sin reescribir `estado`, sin borrar y sin bloquear; la firmeza la calcula `puras.firmeza` al leer (detalle en `specs/gate-investigacion/plan.md`, GI-08 a GI-10) | `investigation/nodos.py::verify` | §4, §11b | G4 |
| **P-73** | Informe del gate de Investigation con el **recuento de hechos por dimensión** y los `no_respaldado` destacados, y en modo exhaustivo una línea por sesión dirigida | `investigation/informe.py` | §4, §18 | G4 |
| **P-74** | Aislamiento de PII: el prompt del investigador no se construye con los campos personales del brief —regla `pii-fuera-del-investigador`— y **todo prompt del investigador ya ensamblado pasa por `pii_en_prompt_de_investigacion` antes de emitirse**; si lleva un dato personal, no sale | `investigation/prompts.py`, `investigation/nodos.py::datos_personales`, `tests/adversarias/test_adversario.py` | §15, spec §4.2 | G1 |
| **P-153** | **Modo exhaustivo de la investigación**: `investigar_exhaustiva` corre en serie las sesiones de `encargos_dirigidos` —seis por dimensión, personajes con evento ancla, oficio— con el perfil `investigador_dirigido` (1 `WebSearch` + 1 `WebFetch`, techo 15.000, cuatro turnos); salta con aviso la sesión con dato personal o salida inválida; los comentarios de rehacer llegan a todas | `investigation/nodos.py::investigar_exhaustiva`, `investigation/nodos.py::encargos_dirigidos`, `investigation/prompts.py::prompt_de_dimension`, `investigation/prompts.py::prompt_de_personajes`, `investigation/prompts.py::prompt_de_oficio`, `commons/agents/techos.py`, `tests/unit/test_investigacion_exhaustiva.py` | §4, §12, §15 | G1 |
| **P-154** | **El modo viaja con la novela**: `Settings.investigacion`, el campo `investigacion` de `EstadoNovela` que `estado_inicial` toma de `Settings`, el flag `--investigacion` de `storymaker nueva` y la elección en `research` | `commons/config.py`, `commons/graph/estado.py`, `commons/graph/run.py`, `cli/comandos.py::nueva`, `investigation/nodos.py::research` | §4, §19 | G1 |

### 6.3 `plotting/`

| # | Entregable | Ficheros y símbolos | Arq. | Gate |
|---|---|---|---|---|
| **P-75** | Nodo `plotting.plan`: el arquitecto **inventa Premisa y Tema**, puebla `canon_*` —personajes, relaciones, **arcos con sus hitos anclados a escenas**, escenarios, voz, glosario— y `plan_*` con la escaleta de capítulos → escenas → beats. **El prompt enuncia los capítulos del brief, de 2 a 4 escenas por capítulo y la extensión**, y lista los elementos del encargo con su clave `#id`. `volcar_escaleta` recibe los mapas de hechos y de datos, resuelve clave o texto y apunta como aviso `anclaje_resuelto` lo que no resuelve; la ficha del homenajeado lleva `nombre_homenajeado` del brief (ER §7.2, §7.4). Rehacer, la revisión guardada y los huecos con escena: `specs/trama-rehacible/plan.md`, TR-01 a TR-14 | `plotting/nodos.py::plan`, `plotting/esquemas.py`, `plotting/escaleta.py`, `plotting/canon.py::volcar_canon` | §4, §7 | G1 |
...
| **P-77** | Nodo `plotting.fill_gap`: **una única micro-llamada** con una `WebSearch`, **tope de cinco huecos** en el estado del grafo, y `no_encontrado` que **autoriza la invención** como fila con `estado='inferido'`, `origen='invencion_autorizada'`, sin fuente y `respaldo='no_aplica'`. El hecho que la micro-sesión encuentra **pasa en el acto por el verificador** con `verificar_hecho`; el inventado no, porque no tiene cita que comprobar (GI-11). Rehacer, la revisión guardada y los huecos con escena: `specs/trama-rehacible/plan.md`, TR-01 a TR-14 | `plotting/nodos.py::fill_gap` | §4 | G1 |
| **P-78** | La invención **no se topa, se cuenta**: el informe del gate de Plotting muestra el recuento de inventados **por dimensión**, y cuántos hechos de la micro-sesión quedaron sin respaldo | `plotting/informe.py` | §4 | G4 |
| **P-79** | Nodo `plotting.seal`: hash sobre el contenido ordenado de las tablas `mundo_*` **vigentes**, con el `respaldo` de cada hecho, fila en `mundo_sello`, y corpus de **solo lectura** a partir de ahí | `plotting/nodos.py::seal` | §4, §7 | G1 |
| **P-80** | Puerta del gate de Plotting: `cobertura_anclada`, `arco_anclado` y **Lean sobre la cronología deducida de la escaleta** —`plan_escena.fecha_narrativa`, `plan_escena_personaje` y las fechas vitales— en verde antes de abrir el gate, y **un aviso por capítulo fuera del rango de 2 a 4 escenas**, más los anclajes que el volcado no resolvió, también como aviso. Rehacer, la revisión guardada y los huecos con escena: `specs/trama-rehacible/plan.md`, TR-01 a TR-14 | `plotting/gate.py` | §11a, §11c | G4 |
...
| # | Entregable | Ficheros y símbolos | Arq. | Gate |
|---|---|---|---|---|
| **P-81** | Nodo `writing.write`: el escritor **redacta el capítulo entero de una vez**, con el paquete de P-33 como única entrada y **sin herramientas**; los encabezados markdown que traiga, de cualquier nivel, se quitan al guardar | `writing/nodos.py::write`, `writing/esquemas.py`, `commons/db/repos/texto.py::sin_encabezados` | §4, §5 | G1 |
...
| **P-83** | Nodo `writing.extract`, **pasada del extractor**, una sola llamada y **solo si la anterior no dejó incidencias**: resumen, delta de continuidad, hechos usados, elementos usados, **eventos de cronología narrativa** y veredicto de ejecución. Recibe la escaleta del capítulo y el **catálogo de identificadores** de ER §7.3, que es también el dominio con el que `schema_guard` valida su salida. Un evento cuya clave ya escribió un intento anterior no se duplica y no cuelga sus participantes de otro evento | `writing/nodos.py::extract`, `writing/extraccion.py::catalogo`, `writing/extraccion.py::volcar_cronologia`, `writing/esquemas.py` | §4 | G3 |
...
| **P-143** | `cobertura_capitulo` **avisa**: abre incidencia de severidad `aviso` que nombra cada elemento que falta por su texto, y el registro la declara no bloqueante. `CapituloEnRevision` lleva `personalizacion_textos`, que rellena `writing/validacion.py` con `intake.texto_de_dato`. La cobertura que bloquea queda en `cobertura_personalizacion` (P-89) | `commons/validation/chapter_validator.py::cobertura_capitulo`, `commons/validation/modelos.py::CapituloEnRevision`, `commons/validation/registro.py`, `writing/validacion.py`, `commons/db/repos/intake.py::texto_de_dato` | §11a | G3 |
...
| **P-152** | El juez **enumera las contradicciones** en `SalidaJuez.contradicciones` antes de puntuar; `judge` topa la continuidad a `10 − 2·n` con mínimo 1 y guarda la lista en el detalle del *score* | `publication/esquemas.py::SalidaJuez`, `publication/nodos.py::_rubrica`, `publication/nodos.py::topar_continuidad`, `publication/nodos.py::judge` | §11b | G5 |
...
| # | Entregable | Ficheros y símbolos | Arq. | Gate |
|---|---|---|---|---|
| **P-95** | Nodo `regeneration.request`: la petición en lenguaje natural se resuelve **por búsqueda semántica** contra canon y corpus, y **el Autor elige en el gate la fila y su valor** (`<objeto>:<fila_id> <campo>=<valor>`), que se aplican sin volver a buscar; sin ellos no se toca nada | `regeneration/nodos.py::request`, `regeneration/cambio.py::resolver_eleccion`, `regeneration/esquemas.py`, `cli/salida.py::candidatos` | §4, §16.2 | G4 |
| **P-96** | **Se modifica la fila del hecho, nunca el texto**, y queda en `audit_log` | `regeneration/cambio.py` | §2 p.4, §4 | G1 |
| **P-97** | Nodo `regeneration.invalidate`: `capitulos_afectados` sale de `uso_hecho` y, para un personaje, de `continuidad`, `plan_escena_personaje` y `uso_hito`; los posteriores pasan a `Invalidado` y se les corren **solo los validadores de coste cero**, Python y Lean, sin invocar al extractor | `regeneration/nodos.py::invalidate` | §4, §11c | G3 |
...
| **P-105** | Interfaz `Notifier` con Telegram como única implementación: `httpx` contra la Bot API, **solo aviso**, en texto plano y terminado en «Decide en el PC», sin comandos; un envío fallido se avisa en la salida y no tumba el gate | `gates/notifier.py` | §10, §16.1 | G1 |
...
| **P-140** | **Avisos fuera de gate**, que salen siempre, también en batch: **parada** cuando la invocación termina en `Fail` —nodo o capítulo y motivo, sin comandos—, **final** cuando llega a `Idle` —versión y coste— y **aparcamiento** cuando `aparcar` aparca un gate, terminado en «Decide en el PC». Pasan por `avisar_sin_fallar`, así que un aviso perdido no cambia el resultado; parar en un gate no emite aviso de parada. `invocar` y `reanudar` aceptan un `notifier` inyectable | `gates/notifier.py::aviso_de_parada`, `gates/notifier.py::aviso_de_terminada`, `gates/notifier.py::aviso_de_aparcada`, `gates/notifier.py::avisar_sin_fallar`, `commons/graph/run.py::_avisar`, `gates/nodos.py::aparcar` | §10 | G1 |

### 9.2 `api/` y `cli/`

| # | Entregable | Ficheros y símbolos | Arq. | Gate |
|---|---|---|---|---|
| **P-138** | La **aplicación de FastAPI** que monta las superficies de §5 —lectura, seguimiento y operación— y registra los manejadores de error de P-128. Existe como pieza propia porque es quien declara que **ninguna ruta reanuda una ejecución** | `api/app.py::crear_app` | §16.4, spec §5 | G1 |
...
| **P-136** | **FastAPI sirve el frontend construido** desde `frontend_dist` y expone su URL base, para que la lectura, el PDF y `render_visual` compartan origen. El `build` del frontend es requisito de la publicación, no del arranque: sin `dist/` el servidor levanta y solo falla la ruta de la aplicación | `api/estaticos.py::montar_frontend`, `commons/config.py::Settings.frontend_dist`, `commons/config.py::Settings.frontend_base_url` | §16.4 | G5 |
| **P-111** | `GET /novelas` que **lista las carpetas de `proyectos/`** y abre el fichero de cada una: no hay base de datos global de novelas. **Una sola función** resuelve `proyectos/<nombre>/<nombre>.db` para la CLI, la API y las evaluaciones, y `crear_novela` crea la carpeta | `api/novelas.py`, `commons/db/apertura.py::ruta_de_novela` | §16.4 | G1 |
| **P-112** | Contrato OpenAPI y Schemathesis sobre la petición de cambio: **una petición malformada no abre la Fase 6**; y una prueba que fija que **las rutas que operan solo aceptan clientes locales y JSON** | `tests/api/test_api.py` | spec §5 | G1 |
...
| **P-113** | Los nueve comandos de Typer: `nueva`, `continuar`, `decidir`, `estado`, `ramificar`, `cambiar`, `desbloquear`, `reintentar` —cuyo contenido es P-144—, `evaluar`. `nueva` **abre el fichero de encargo** y lo redacta como la premisa con la que arranca la Fase 1: no escribe un `Brief`, para que el encargo completo y el de cuatro líneas entren por el mismo camino | `cli/comandos.py`, `intake/encargo.py::leer` | §16.4, spec §6 | G1 |
| **P-144** | `storymaker reintentar <novela>`: sobre una novela terminada en `Fail` de capítulo —checkpoint sin nodo siguiente, sin gate pendiente, corpus sellado—, escribe en el checkpoint un capítulo recién empezado como salida de `SealCorpus` y reanuda por `invocar`. Fuera de ese caso lanza `NadaQueReintentar` sin tocar nada | `commons/graph/run.py::reintentar`, `commons/errores.py::NadaQueReintentar`, `cli/comandos.py::reintentar` | §16.5 | G1 |

### 9.3 La interfaz opera el arnés

Lo que la API necesita para que el Autor encargue, siga, decida y continúe una novela desde la interfaz (arq. §16.5, spec §5.2 y §5.3). **Lo que ejecuta el grafo lo sigue ejecutando la CLI**: estos ítems la lanzan, no la sustituyen. Los números saltan a `P-160` para no pisar los que otra línea de trabajo numera en paralelo, que llegó hasta `P-152`.

| # | Entregable | Ficheros y símbolos | Arq. | Gate |
|---|---|---|---|---|
| **P-160** | **Seguimiento**: el estado de una novela con la precedencia de spec §5.2, incluida la carpeta con encargo y sin `.db`; el estado de cada fase desde `fase_run` o, sin filas, desde su salida, marcado como deducido; el panel —fases, gate, capítulos con intentos, actividad interpretada, consumo, proceso vivo, registros—; el gate pendiente con sus recuentos, preguntas, petición y candidatos de Regeneración, y filas editables; y `GET /novelas` extendido. Todo se recalcula del fichero en cada consulta | `api/seguimiento.py::estado_de`, `api/seguimiento.py::fases_de`, `api/seguimiento.py::panel`, `api/seguimiento.py::gate`, `api/seguimiento.py::listar`, `tests/api/test_seguimiento.py` | §16.5 | G1 |
| **P-161** | **La salida de cada fase** con sus ejecuciones y las decisiones de sus gates —Encargo, Investigación, Trama, Escritura, Publicación y Regeneración—, el texto de un intento con sus incidencias y el final de un registro de proceso | `api/fases.py::salida`, `api/fases.py::intento`, `api/fases.py::registro`, `tests/api/test_fases.py` | §16.5 | G1 |
| **P-162** | **Operación**: `POST /novelas`, `continuar`, `decisiones`, `reintentar`, `desbloquear` y `ediciones`, más `POST /encargos/validar` y `GET /ejemplos`. Las cuatro primeras **lanzan la CLI** desacoplada del servidor, con una consola oculta que heredan sus hijos para no abrir ventanas, con su registro en `proyectos/<nombre>/registro/`, responden `202` y dejan la acción en `audit_log`; comprueban antes lo que el comando rechazaría. Desbloquear solo con el proceso muerto. Las ediciones toman el cerrojo y usan `regeneration/cambio.py::aplicar`. Todas rechazan con `403` a un cliente no local y con `415` un cuerpo no JSON | `api/operacion.py::router`, `api/lanzador.py::lanzar`, `api/guardas.py::solo_local`, `tests/api/test_operacion.py` | §16.5, §10 | G1 |
| **P-163** | `proceso_vivo`: si el PID que guarda el cerrojo sigue vivo, **sin matar nada** —en Windows con `OpenProcess` y `GetExitCodeProcess`, porque `os.kill(pid, 0)` termina el proceso allí— | `commons/graph/cerrojo.py::proceso_vivo`, `commons/graph/cerrojo.py::pid_del_cerrojo` | §16.4, §16.5 | G1 |
| **P-164** | `python -m storymaker` como punto de entrada de la CLI, para que el lanzador use el mismo intérprete que el servidor | `__main__.py` | §16.5 | G1 |
| **P-165** | `decidir` **rechaza `abortar` fuera del gate de Intake** sin tocar nada, que es lo que declara el modelo TLA+ | `gates/decisiones.py::aplicar` | §10 | G1 |
| **P-166** | La petición de cambio del lector queda en `audit_log` **con su texto, fragmento, capítulo, versión y candidatos**, para que el gate los enseñe sin repetir la búsqueda | `api/cambios.py::pedir_cambio` | §16.5 | G1 |
| **P-180** | El encargo admite **`descripcion`**, que `redactar` antepone a la premisa en prosa con las palabras de quien encarga; no pasa por la cuarentena | `intake/encargo.py::redactar`, `tests/unit/test_encargo.py` | §4 F1 | G1 |
| **P-181** | En Intake, el gate devuelve **la conversación**: la descripción del encargo, los comentarios de los gates de Intake decididos como «rehacer», en orden, y el brief si ya está cerrado | `api/seguimiento.py::conversacion_de`, `tests/api/test_seguimiento.py` | §4 F1, §16.5 | G1 |
| **P-182** | `POST /novelas` acepta `investigacion` —`estandar` o `exhaustiva`— y la pasa a `storymaker nueva` como `--investigacion exhaustiva` | `api/operacion.py::encargar`, `tests/api/test_operacion.py` | §4 F2, §16.5 | G1 |
| **P-183** | **Nombre corto de cada escenario**: el del lugar o el arranque de la descripción, seis palabras como mucho, en la ficha versionada y en la salida de la Trama | `api/lectura.py::nombre_corto`, `api/fases.py::_trama`, `tests/api/test_api.py` | §16.5 | G1 |
| **P-184** | `GET /novelas/{id}/versiones/{n}/pdf`: el `<nombre>.v<n>.pdf` de la carpeta de la novela, como descarga, o `404` | `api/lectura.py::pdf`, `tests/api/test_api.py` | §4 F5, §16.5 | G1 |
| **P-185** | **El PDF desde la ruta de impresión del frontend**: `imprimir_version` abre `/novelas/<n>/v/<k>/imprimir` en Playwright y responde sus peticiones con los estáticos de `frontend_dist` y la API en memoria (ASGI); `imprimir_pendientes` imprime las versiones sin PDF y cae al HTML mínimo sin `dist/`. La llama `invocar` tras el `commit` final, si termina en `Idle` y antes de avisar, como aviso si falla; `publish` deja de imprimir dentro del paso | `publication/render.py::imprimir_version`, `publication/render.py::imprimir_pendientes`, `commons/graph/run.py::invocar`, `publication/nodos.py::publish`, `tests/unit/test_impresion.py` | §4 F5, §16.1 | G5 |
| **P-186** | **Los candidatos de Regeneración, elegibles desde el gate**: `GET /gate` sirve cada candidato con `objeto`, `fila_id`, `campo` —el de `CAMPO_POR_DEFECTO`— y `valor` —su descripción guardada—, y los deja vacíos si la petición se registró sin fila. La elección la aplica P-95 | `api/seguimiento.py::Candidato`, `api/seguimiento.py::_peticion`, `tests/api/test_seguimiento.py` | §4 F6 | G1 |

### 9.4 Cierre de verificación
...
| **P-119** | CrossHair sobre las cinco funciones puras y **`mutmut` con mutación ≥ 80 %** en `commons/validation/` | `[tool.mutmut]` de `backend/pyproject.toml` | verif. §3.3, §3.7 | G2 |
...
| Fecha | Cambio | Motivo |
|---|---|---|
| 2026-09-24 | **P-05**, **P-63** y **P-119** dejan de nombrar `.github/workflows/ci.yml` y `nocturna.yml`, que se retiran; P-05 remite a los comandos de verif. §4.7 y añade `--skip-editable` a `pip-audit`, y P-63 nombra `formal/tla/harness.cfg` | Decisión del Autor: se retiran los workflows de GitHub Actions y G1 y G2 pasan a lanzarse a mano (arq. §17, verif. §4.7). Con `--strict` y sin `--skip-editable`, `pip-audit` fallaba siempre al intentar auditar el propio paquete |
| 2026-09-24 | **P-75**, **P-77** y **P-80** remiten a `specs/trama-rehacible/plan.md` para rehacer la Trama, la revisión guardada y los huecos con escena | Se propagan §4 (Fase 3) de la arquitectura y REQ-BE-217 a REQ-BE-219 |
| 2026-09-24 | **P-68**: el `n_capitulos` del brief cerrado pasa al estado | Se propagan §4 de la arquitectura y REQ-BE-216 |
| 2026-09-24 | **P-18** gana la columna aditiva `sin_respaldo` y **P-72** el veredicto parcial. El resto del cambio —lagunas, estados respecto a la fuente, uso de la firmeza, aviso de escenas poco firmes— vive en `specs/gate-investigacion/plan.md`, GI-18 a GI-29 | Se propagan la arquitectura §4, §6, §7 y §17 y REQ-BE-207 a REQ-BE-213 |
| 2026-09-24 | **P-72**, **P-77**, **P-78** y **P-79**: el verificador escribe solo `respaldo`, los hechos de la micro-sesión pasan por él, el informe de Plotting cuenta los que quedan sin respaldo y el sello incluye el respaldo. La forma exacta vive en `specs/gate-investigacion/plan.md`, GI-08 a GI-17 | Se propagan la arquitectura §4 y §7 y REQ-BE-199 a REQ-BE-205 |
| 2026-09-24 | **P-105** y **P-140**: los avisos no traen comandos. El de un gate y el de aparcamiento terminan en «Decide en el PC», y el de parada dice nodo y motivo sin `estado` ni `continuar` | Se propagan la arquitectura §10, REQ-BE-103 y la spec de ejecución real §9.5 |
| 2026-09-24 | **P-95**: el Autor elige en el gate la fila y su valor, y sin ellos no se toca nada; **P-97**: el alcance de un personaje sale de `continuidad`, la escaleta y `uso_hito` | Se propagan §4 de la arquitectura y REQ-BE-196 a REQ-BE-198 |
| 2026-09-24 | Entra **P-186**: los candidatos del gate de Regeneración con lo que hace falta para elegirlos | Baja de §5.2 de la spec |
| 2026-09-24 | Entra **P-185**: el PDF desde la ruta de impresión del frontend, impreso tras confirmar la invocación. Cierra la parte del PDF de P-94 | Baja de §4.5 de la spec |
| 2026-09-24 | Entra **P-184** (el PDF de cada versión por la API) y P-162 precisa que el proceso lanzado lleva una consola oculta | Baja de §5.1 y §5.3 de la spec |
| 2026-09-24 | **P-69**: la denegación por cuota pide entregar ya, veinte turnos, y la sesión sin respuesta válida no detiene la fase | Se propagan §4 de la arquitectura y REQ-BE-194 y REQ-BE-195 |
| 2026-09-24 | Entra **P-183**: el nombre corto de los escenarios | Baja de §5.1 de la spec |
| 2026-09-24 | Entran **P-153** —el modo exhaustivo de la investigación— y **P-154** —el modo viaja con la novela—; **P-73** enseña una línea por sesión dirigida y **P-74** conecta la guarda de PII a todo prompt del investigador | Se propagan §4, §12 y §15 de la arquitectura y REQ-BE-148, REQ-BE-149 y REQ-BE-190 a REQ-BE-193 de la spec |
| 2026-09-24 | Entra **P-182**: el modo de investigación al encargar desde la interfaz | Baja de §5.3 de la spec |
| 2026-09-24 | Entran **P-180** (la descripción del encargo como premisa) y **P-181** (la conversación del gate de Intake) | Baja de §4.1 y §5.2 de la spec: el encargo por conversación. Bloque 180–189 acordado con la otra línea de trabajo |
| 2026-09-24 | Los ítems de §9.3 pasan de `P-150`–`P-156` a **`P-160`–`P-166`**, y con ellos sus citas en las dos specs, el plan del frontend, las tres matrices y el registro de iteraciones | Dos líneas de trabajo en paralelo usaron `P-150` a `P-152` a la vez: los del contexto de trama (It-26) y los de la interfaz. Un identificador no se reutiliza, así que se movió el bloque que menos citas tenía fuera del backend |
| 2026-09-24 | **P-41**: `guardrail_prohibidas` con derivadas; **P-81**: los encabezados markdown del escritor no se guardan; **P-68**: el evento ancla se vuelca como elemento obligatorio | Se propagan §4 y §11a de la arquitectura y REQ-BE-140 a REQ-BE-142 |
| 2026-09-24 | Entra **§9.3, la interfaz opera el arnés**: P-160 (seguimiento), P-161 (salida de cada fase), P-162 (operación con el lanzador de la CLI y las guardas), P-163 (`proceso_vivo`), P-164 (`python -m storymaker`), P-165 (abortar solo en Intake) y P-166 (la petición del lector en `audit_log`). P-112 deja de fijar que ninguna ruta reanuda y fija que las que operan solo aceptan clientes locales; P-138 monta las tres superficies. La antigua §9.3 pasa a §9.4 | Baja de arq. §16.5 y de §5 de la spec. Los números saltan a 150 para no pisar los que otra línea de trabajo numera en paralelo |
| 2026-09-24 | **P-41**: `nombres_exactos` admite la mayúscula inicial de principio de frase | Se propagan §11a de la arquitectura y REQ-BE-139 de la spec |
| 2026-09-24 | Entran **P-150** y **P-151**, la trama anterior y lo ya gastado en el paquete del escritor, y **P-152**, las contradicciones del juez | Se propagan §6 y §11b de la arquitectura (REQ-BE-145 a REQ-BE-147) |
| 2026-09-24 | Entran **P-143**, `cobertura_capitulo` como aviso con el texto del elemento, y **P-144**, `storymaker reintentar` | Se propagan §11a y §16.5 de la arquitectura (REQ-BE-137 y REQ-BE-138). La cuarta novela real agotó los reintentos del capítulo 8 con el mismo texto tres veces y no tenía forma de seguir |
| 2026-09-24 | Entran **P-141** y **P-142**: una `fase_run` por fase abierta por un envoltorio común a los nodos, `corpus_run_id` y el consumo contado en el transporte | Se propaga la spec de ejecución real §9.1 y §9.2 (REQ-ER-25, REQ-ER-26 y REQ-ER-36), que deja de apuntar a P-57 y P-60: ninguno de los dos realizaba la contabilidad |
| 2026-09-24 | Entra **P-140**, los avisos fuera de gate —parada, final y aparcamiento—; **P-83** recoge que un evento narrativo con clave ya escrita no cuelga sus participantes de otro | Se propagan la arquitectura §10 y la spec de ejecución real §9.5 y §7.3 (REQ-ER-32 a REQ-ER-35). Los identificadores continúan la numeración: un `P-nn` no se reutiliza |
| 2026-09-24 | **P-111** resuelve y lista una carpeta por novela con `ruta_de_novela`, compartida por CLI, API y evaluaciones | Se propaga REQ-BE-136, decisión del Autor en §16.4 |
| 2026-09-24 | **P-75** pasa los mapas de anclaje a `volcar_escaleta`, lista los elementos del encargo con clave y guarda al homenajeado con el nombre del brief; **P-80** enseña los anclajes sin resolver como aviso | Spec de ejecución real §7.2 y §7.4: la segunda novela real salió con `plan_anclaje` vacía y la homenajeada abreviada en el canon |
| 2026-09-24 | **P-75** enuncia en el prompt del arquitecto capítulos, escenas por capítulo y extensión; **P-80** avisa de los capítulos fuera del rango de escenas | Se propaga REQ-BE-135: la escaleta salía con una escena por capítulo porque el rango de §19 no lo usaba nada |
| 2026-09-24 | **P-68** hace pasar la entrevista por el gate de Intake | Se propaga la decisión de la arquitectura en la Fase 1 |
| 2026-09-24 | **P-31** acepta un contexto de validación y **P-83** entrega al extractor su escaleta y el catálogo de identificadores, que es el dominio de su salida | Spec de ejecución real §7.3: la segunda novela real se detuvo por una clave foránea porque el extractor adivinaba los identificadores |
...
| 2026-09-24 | **P-129** lee también las filas `IMP-nn` y recorre `frontend/src/**` en la dirección inversa; **P-136** nombra `Settings.frontend_base_url`. Son las contrapartes de `FE:IMP-33` y `FE:IMP-26` | Baja de §7.1 nº 21 y de REQ-BE-113 de la spec. El parser solo recogía filas `P-nn`, así que el plan del frontend estaba fuera del inventario aunque este ítem prometía leer todos los planes |
