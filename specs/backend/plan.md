...
| **P-105** | Interfaz `Notifier` con Telegram como única implementación: `httpx` contra la Bot API y **botones inline** *Aprobar · Rehacer · Abortar* | `gates/notifier.py` | §10, §16.1 | G1 |
...
| # | Entregable | Ficheros y símbolos | Arq. | Gate |
|---|---|---|---|---|
| **P-138** | La **aplicación de FastAPI** que monta las tres superficies de §5 —webhook, lectura y petición de cambio— y registra los manejadores de error de P-128. Existe como pieza propia porque las tres tienen perfiles de riesgo distintos y solo una reanuda una ejecución: quien las monta declara cuál va protegida | `api/app.py::crear_app` | §16.4, spec §5 | G1 |
...
| **P-112** | Contrato OpenAPI y Schemathesis sobre el endpoint de decisión: **una decisión malformada no reanuda el grafo** | `tests/api/test_api.py` | spec §5 | G1 |
